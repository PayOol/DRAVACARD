...
export function validateCommonWorker(sources) {
  ...
  requireAll(failures, notifications, ['secret(env.EMAILJS_SERVICE_ID)', 'secret(env.EMAILJS_TEMPLATE_ID)', 'secret(env.EMAILJS_PUBLIC_KEY)', '/^[a-fA-F0-9]{64}$/.test(env.TIKTOK_DATA_KEY??"")', 'crypto.subtle.encrypt(', 'crypto.subtle.decrypt(', 'name:"AES-GCM"', 'additionalData:newTextEncoder().encode(additionalData)', 'crypto.getRandomValues(newUint8Array(12))', '"https://api.emailjs.com/api/v1.0/email/send"', 'redirect:"manual"', 'signal:controller.signal', 'response.body?.cancel()', 'returnresponse.ok'], 'Shared notification transport must retain the reviewed EmailJS configuration, AES-GCM envelopes, fixed endpoint, redirect policy and timeout')
  for(const operation of ['sealNotification','unsealNotification']) requireAll(failures,functionCode(notifications,operation),['name:"AES-GCM"','additionalData:newTextEncoder().encode(additionalData)'],`Shared ${operation} must authenticate caller-provided order context`)
  requireAll(failures, tiktok, ['awaitseal(client,order.orderId,env)', 'awaitseal({username},`receipt:${order.orderId}`,env)', 'awaitunseal(sealed,order.orderId,env)', 'awaitunseal(receipt,`receipt:${order.orderId}`,env)', 'expirationTtl:NOTIFICATION_TTL_SECONDS', 'sendOrderEmail(env,{service_type:"RechargeTikTokCoins"', 'if(!accepted)returnresult("pending")', 'awaitenv.ORDERS.delete(`${storageKey}:customer`)'], 'TikTok credentials must retain their exact historical AAD, payload and delivery lifecycle')
  requireAll(failures, functionCode(services,'prepareFulfillment'), ['sealNotification({email:client.email,whatsapp:client.whatsapp},`cards:${key}`,env)', 'expirationTtl:NOTIFICATION_TTL_SECONDS'], 'Card contacts must be encrypted with a stable isolated order context and finite TTL')
  requireAll(failures, functionCode(services,'notifyCardOrder'), ['if(!sealed)return{}', 'if(!emailReady(env))return{notification:"pending"}', 'unsealNotification(sealed,`cards:${key}`,env)', 'service_type:"Cartevirtuelle"', 'order_id:order.orderId', 'client_email:client.email', 'client_whatsapp:client.whatsapp', 'price:newIntl.NumberFormat("fr-FR").format(order.amount)', 'date:newDate(order.createdAt).toISOString()', 'card_name:PRODUCTS[order.productId].name', 'if(!accepted)return{notification:"pending"}', 'awaitenv.ORDERS.put(`${key}:notified`,"sent"', 'awaitenv.ORDERS.delete(`${key}:customer`)'], 'Verified card delivery must use only canonical order/contact fields, retry safely and omit invented legacy data')
