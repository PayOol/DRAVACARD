...
Le composant `PwaInstallPrompt` vit dans les mêmes providers de langue et de thème que l’application. Sa disponibilité dépend de la page, des dialogues, de la fiche mobile, du focus et du marqueur `data-drava-checkout-active` de `Home`. Ce marqueur reste actif de la sélection jusqu’à la fermeture, y compris pendant le chargement différé du module et les résultats intégrés. Conserver cette protection lors de l’ajout d’un service. Le chargement des modules ne déplace pas les règles de consentement ou de paiement dans les catalogues. L’interface de confirmation des mises à jour a été supprimée à la demande de l’utilisateur.

L’invite reprend le rappel de deux heures d’UpCoin et respecte les deux layouts. L’installation reste explicite. Avant ouverture, elle vérifie le mode application, le marqueur d’installation persistant par identité et l’API des applications liées lorsqu’elle est disponible. Une installation confirmée ferme l’invite et synchronise les onglets ; un nouvel événement natif peut permettre une réinstallation. Le texte « Drava s’ouvrira… » est retiré des deux langues. Safari/iOS ne garantit pas la détection d’une app déjà installée depuis un onglet classique.

Les mises à jour sont automatiques et silencieuses sur les deux layouts. La recherche se fait à l’enregistrement, périodiquement toutes les dix minutes sur une page visible, puis est réévaluée au réveil et au retour du réseau. L’activation exige trois secondes sans interaction et un accord technique de tous les onglets du périmètre, avec prise en charge du rechargement automatique. Une commande, un dialogue, un champ actif ou modifié, une composition clavier, une fiche `#card:…`, une URL privée ou avec paramètres bloque le passage. Un ancien onglet sans le nouveau protocole doit être navigué ou rouvert avant de permettre l’activation silencieuse.

Après changement de contrôleur, tous les onglets compatibles se rechargent uniquement lorsqu’ils sont disponibles. Si une saisie ou une commande commence entre l’accord et ce changement, le besoin de rechargement reste en mémoire et attend sa fermeture ; aucune interaction n’est interceptée. La première installation du worker ne recharge pas la page. Une application fermée ou un onglet suspendu ne peut pas garantir l’exécution immédiate de ces contrôles.

Les ressources mises en cache sont publiques, bornées et vérifiées contre les empreintes de la construction. Les états publics `waiting`, `active` et `previous` évitent qu’une version en attente remplacée ne chasse les ressources encore utilisées par la version active. Les requêtes et réponses privées, métadonnées clients, navigations et résultats de paiement restent exclus. Voir [PWA.md](PWA.md) pour la construction, l’aperçu, les validations courantes et les campagnes historiques.
